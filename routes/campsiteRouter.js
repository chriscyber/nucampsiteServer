...
campsiteRouter
  .route("/") //root path consolidated in server.js
  .get((req, res, next) => {
    Campsite.find()
      .populate("comments.author") // populates author field in comment subdocument based on userId stored in?
      .then((campsites) => {
        res.statusCode = 200;
    ...
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        if (campsite) {
          ...
          campsite.comments.push(req.body); // save to memory, not to db yet
          campsite
            .save() // non-static method to save to db
            .then((campsite) => {
              res.statusCode = 200;
              res.setHeader("Content-Type", "application/json");
              res.json(campsite);
      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          if (
            ...
            res.statusCode = 403;
            res.end("You must be the author of the comment ");
          }
        } else if (!campsite) {
          err = new Error(`Campsite ${req.params.campsiteId} not found`);
          err.status = 404;
